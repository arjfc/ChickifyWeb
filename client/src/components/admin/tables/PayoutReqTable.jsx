...
const baseModalStyle = {
  content: {
    ...
    borderRadius: 20, padding: 20, maxHeight: "90vh", width: "60vw", overflow: "auto",
  },
...
export default function PayoutReqTable({ selectedOption, date = "all" }) {
  ...
  const inRange = useCallback((iso, range) => {
    ...
    switch (range) {
      case "today":
        return d.isAfter(todayStart.subtract(1, "millisecond")) && d.isBefore(todayEnd.add(1, "millisecond"));
      case "yesterday": {
        const yStart = todayStart.subtract(1, "day");
        const yEnd = todayEnd.subtract(1, "day");
        return d.isAfter(yStart.subtract(1, "millisecond")) && d.isBefore(yEnd.add(1, "millisecond"));
      }
      case "7": {
        const start = now.startOf("day").subtract(6, "day");
        return d.isAfter(start.subtract(1, "millisecond")) && d.isBefore(todayEnd.add(1, "millisecond"));
      }
      case "30": {
      ...
      case "last_month": {
        ...
        return d.isAfter(lastMonthStart.subtract(1, "millisecond")) && d.isBefore(lastMonthEnd.add(1, "millisecond"));
      }
      default:
        return true;
    }
  ...
  const payoutReqData = useMemo(() => {
    const byTab =
      selectedOption && selectedOption !== "All"
        ? mapped.filter((c) => String(c.status).toLowerCase() === String(selectedOption).toLowerCase())
        : mapped;
  ...
  const showProcessedColumn = String(selectedOption || "All").toLowerCase() !== "pending";

  const headers = showProcessedColumn
    ? ["Payout ID", "Requestor (Farmer)", "Amount", "Request Date", "Processed At", "Status", "Action", ""]
    : ["Payout ID", "Requestor (Farmer)", "Amount", "Request Date", "Status", "Action", ""];
  ...
  const selectedItems = useMemo(
    () => payoutReqData.filter((row, i) => isSelectableStatus(row.status) && selected[i]),
  ...
  const viewDetails = (row) => { setModalError(""); setModalData(row); setIsModalOpen(true); };
  ...
  const confirmPayout = useCallback(async () => {
    ...
    try {
      ...
      const { data: pub } = supabase.storage.from("payout-proofs").getPublicUrl(path);
  ...
  }, [canConfirmPayout, selectedItems, payoutImageFile, payoutAmount, gcashName, gcashNumber, loadRows, resetApprovePayoutModal]);
  ...
  const openRejectConfirm = (ids) => { setRejectIds(ids); setRejectError(""); setRejectModalOpen(true); };
  const closeRejectConfirm = () => { setRejectModalOpen(false); setRejectIds([]); setRejectError(""); };
  ...
  const confirmRejectNow = useCallback(async () => {
    ...
    try {
      ...
      setSelectAll(false);
      closeRejectConfirm();
      if (isModalOpen) { setIsModalOpen(false); setModalData(null); }
    } catch (e) {
  ...
  useEffect(() => {
    const handler = () => { void rejectSelectedDirect(); };
  ...
  useEffect(() => {
    const openApprove = () => { if (selectedItems.length > 0) setIsPayoutOpen(true); };
  ...
  return (
    <Table headers={headers}>
      {loading && (
        <tr>
          <td className="px-4 py-3 text-center text-gray-500" colSpan={headers.length}>
            Loading payouts…
          </td>
        </tr>
      )}
      {err && !loading && (
        <tr>
          <td className="px-4 py-3 text-center text-red-600" colSpan={headers.length}>
            {err}
          </td>
        </tr>
      )}
      ...
      {!loading && !err && payoutReqData.map((item, index) => {
        const { raw: rawStatus, label: statusLabel } = formatStatus(item.status);
        const showProcessedForRow =
          rawStatus === "approved" || rawStatus === "rejected" || rawStatus === "processing";

        return (
          <tr key={index} className="bg-yellow-100 text-gray-700 rounded-lg shadow-sm transition">
            ...
            <td className={`px-4 py-3 text-center font-medium ${statusColorClass(rawStatus)}`}>
              {statusLabel}
            </td>
      ...
      >
        {modalData && (() => {
          const { raw: rawStatus, label: statusLabel } = formatStatus(modalData.status);
          const showProof = rawStatus === "approved";
          const showProcessedBlock = rawStatus === "approved" || rawStatus === "rejected" || rawStatus === "processing";

          return (
            <div className="flex flex-col gap-4 p-6">
              <div className="flex flex-row justify-between items-center">
                <div className="flex flex-col">
                  <h1 className="text-2xl font-bold text-primaryYellow">Payout Details</h1>
                  <p className="text-lg text-gray-400 font-bold">Payout ID: {modalData.payoutID}</p>
                </div>
                <div className="flex flex-col items-end">
                  <h1 className="text-lg text-gray-400 font-bold">Current Status:</h1>
                  <p className={`text-2xl font-bold ${statusColorClass(rawStatus)}`}>{statusLabel}</p>
                </div>
              </div>
              ...
              <div className="border-2 p-5 rounded-lg w-full flex gap-8 items-start">
                {showProof && (
                  <div className="flex flex-col items-center w-1/2 -ml-8">
                    ...
                    {modalData.proofImageUrl ? (
                      <img src={modalData.proofImageUrl} alt="Proof" className="rounded-lg border" style={{ width: 300, height: "auto" }} />
                    ) : (
                ...
                <div className={`flex flex-col gap-3 ${showProof ? "w-1/2 -ml-4" : "w-full"}`}>
                  <div className="grid grid-cols-2 gap-8">
                    <div>
                      <p className="font-bold text-primaryYellow">Requestor</p>
                      <p className="text-gray-500 font-bold">{modalData.requestor}</p>
                    </div>
                    <div>
                      <p className="font-bold text-primaryYellow">Request Date</p>
                      <p className="text-gray-500 font-bold">{modalData.requestDate}</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-8">
                    <div>
                      ...
                      <p className="text-gray-700 font-extrabold">₱{modalData.amount.toLocaleString()}</p>
                    </div>
                    <div>
                      <p className="font-bold text-primaryYellow">Payout ID</p>
                      <p className="text-gray-500 font-bold">{modalData.payoutID}</p>
                    </div>
                  </div>
                  ...
                  {showProcessedBlock && (
                    <div className="grid grid-cols-2 gap-8 border-t pt-4">
                      <div>
                        <p className="font-bold text-primaryYellow">Processed At</p>
                        <p className="text-gray-500 font-bold">{modalData.processedAt || "—"}</p>
                      </div>
                      <div>
                        <p className="font-bold text-primaryYellow">Processed By</p>
                        <p className="text-gray-500 font-bold">{modalData.processedBy ?? "—"}</p>
                      </div>
                    </div>
                  )}
                </div>
      ...
      <Modal
        isOpen={isPayoutOpen}
        onRequestClose={() => { setIsPayoutOpen(false); resetApprovePayoutModal(); }}
      ...
      >
        ...
        <div className="px-6 py-5" style={{ backgroundColor: "#fec718" }}>
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-extrabold text-gray-900">Process Payout</h1>
              <p className="text-sm text-gray-800/80 mt-1">Attach a single image as proof.</p>
            </div>
            <button
              onClick={() => { setIsPayoutOpen(false); resetApprovePayoutModal(); }}
        ...
        {/* Body */}
        <div className="p-6">
          {!!actionError && <div className="mb-4 text-sm font-semibold text-red-600">{actionError}</div>}
          ...
          {/* Image Proof */}
          <label className="block text-sm font-semibold text-gray-700 mb-2">Image Proof (required)</label>
          <div
            onClick={() => { if (!payoutImagePreview) fileInputRef.current?.click(); }}
            className="group relative flex cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed p-6"
            style={payoutImagePreview
          ...
          >
            ...
            <input ref={fileInputRef} type="file" accept="image/*" onChange={onPickPayoutImage} className="hidden" />

            {payoutImagePreview && (
              <div className="mt-4 w-full">
                <div className="rounded-xl border bg-white p-3 shadow-sm">
                  <img src={payoutImagePreview} alt="Payout Proof Preview" className="rounded-lg max-h-64 w-full object-contain" />
                  <div className="mt-3 flex items-center justify-between">
                    <span className="text-xs font-medium text-gray-600">{payoutImageFile?.name ?? "Selected Image"}</span>
                    <div className="flex items-center gap-3">
                      <button onClick={(e) => { e.stopPropagation(); fileInputRef.current?.click(); }} className="text-xs font-semibold text-gray-700">
                        Change image
                      </button>
                      <button onClick={(e) => { e.stopPropagation(); clearPayoutImage(); }} className="text-xs font-semibold text-red-600">
                        Remove
                      </button>
                    </div>
                  </div>
          ...
          <div className="mt-6 flex items-center justify-end gap-3 sticky bottom-0 bg-white py-4 px-0">
            ...
            <button
              onClick={confirmPayout} disabled={!canConfirmPayout}
              className="px-5 py-2 rounded-xl text-white font-semibold shadow"
              style={{ backgroundColor: canConfirmPayout ? "#16a34a" : "#86efac", cursor: canConfirmPayout ? "pointer" : "not-allowed", opacity: isConfirming ? 0.8 : 1 }}
            >
      ...
      >
        <div className="p-6">
          <h2 className="text-xl font-extrabold text-gray-900">Reject Payout Request</h2>
          <p className="mt-2 text-sm text-gray-600">
            {rejectIds.length === 1
              ? <>Are you sure you want to reject payout <span className="font-semibold">#{rejectIds[0]}</span>?</>
              : <>Are you sure you want to reject these <span className="font-semibold">{rejectIds.length}</span> payouts?</>}
          </p>

          {rejectIds.length > 1 && (
            <div className="mt-3 max-h-32 overflow-auto rounded border bg-gray-50 p-2 text-xs text-gray-700">
              {rejectIds.map((id) => (<div key={id} className="py-0.5">• #{id}</div>))}
            </div>
          )}
          ...
          <div className="mt-6 flex items-center justify-end gap-3">
            <button onClick={closeRejectConfirm} disabled={rejectBusy} className="px-4 py-2 rounded-lg bg-gray-200 text-gray-800 font-semibold">
              Cancel
            </button>
            <button onClick={confirmRejectNow} disabled={rejectBusy} className="px-4 py-2 rounded-lg bg-red-600 hover:bg-red-700 text-white font-semibold">
