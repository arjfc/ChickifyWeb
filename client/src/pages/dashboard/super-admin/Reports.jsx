// pages/super-admin/reports/index.jsx
          ...
          return (
            <button
              ...
              className={[
                ...
                (isStart || isEnd) && "bg-primaryYellow text-white hover:bg-primaryYellow",
              ]
                .filter(Boolean)
                .join(" ")}
            >
...
function DateRangePopover({ open, onClose, value, onChange }) {
  ...
  useEffect(() => {
    ...
    setCursor(new Date((parseYMD(value.from) || new Date()).getFullYear(), (parseYMD(value.from) || new Date()).getMonth(), 1));
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-start justify-center pt-24">
      ...
      >
        ...
        {/* Calendars */}
        <div className="flex gap-6 p-4" onMouseLeave={() => setHovering(null)}>
          <div className="flex flex-col gap-2">
            <div className="text-center text-sm text-gray-300">
              {cursor.toLocaleString(undefined, { month: "long", year: "numeric" })}
            </div>
            <MonthGrid monthDate={cursor} start={start} end={end} hovering={hovering} onPick={onPick} />
          </div>

          <div className="flex flex-col gap-2">
            <div className="text-center text-sm text-gray-300">
              {addMonths(cursor, 1).toLocaleString(undefined, { month: "long", year: "numeric" })}
            </div>
            <MonthGrid monthDate={addMonths(cursor, 1)} start={start} end={end} hovering={hovering} onPick={onPick} />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between p-4 border-t border-gray-700">
          ...
          <div className="flex items-center gap-2">
            <button className="px-4 py-2 text-sm rounded-md bg-gray-700 hover:bg-gray-600" onClick={onClose}>
              Cancel
            </button>
            <button
...
export default function Reports() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 border-b border-gray-300 pb-2">
        {/* Tabs */}
        <div className="flex items-center">
          {options.map((lbl) => (
            <button
              key={lbl}
              onClick={() => setSelectedOption(lbl)}
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="bg-primaryYellow text-white text-sm font-semibold rounded-lg px-4 py-2 h-9"
          >
            Generate Report
          </button>
        </div>
