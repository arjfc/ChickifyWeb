...
export async function fetchEggBatchesGrouped({
  ...
  const { data, error } = await supabase.rpc(
    "admin_view_batches_per_farmer_grouped_live",
    {
      ...
      p_date_from: from,
      p_date_to: to,
      p_only_manual: onlyManual, // keep this if your RPC still has this arg
    }
  );
  if (error) throw error;
  ...
  return (data || []).map((r) => {
    ...
    // RPC now returns JSONB array of objects:
    // [{ size, eggs, allocatable, poolable }, ...]
    const sizeBreakdown = Array.isArray(r.size_breakdown)
      ? r.size_breakdown.map((s) => ({
          size: String(s.size || "").toUpperCase(),
          qty: Number(s.allocatable ?? s.trays ?? 0), // allocatable trays
          eggs: Number(s.eggs || 0),
          poolable: Number(s.poolable ?? 0),
        }))
      : [];

    const eggsPerPiece = Number(r.eggs_per_piece || 0);
    const extraTraysFromPieces = Number(r.extra_trays_from_pieces || 0);
    const leftoverPieces = Number(r.leftover_pieces || 0);

    // For the nice message:
    const pieceWord = eggsPerPiece === 1 ? "piece" : "pieces";
    const trayWord = extraTraysFromPieces === 1 ? "tray" : "trays";
    const leftoverWord = leftoverPieces === 1 ? "piece" : "pieces";

    const looseMessage =
      eggsPerPiece > 0
        ? `You have ${eggsPerPiece} ${pieceWord} of eggs → ${extraTraysFromPieces} extra ${trayWord}, ${leftoverPieces} leftover ${leftoverWord}`
        : "";
    ...
    return {
      ...
      // total allocatable (by trays, all sizes)
      trayStocks: Number(r.stocks_tray || 0),
      // new fields from RPC
      eggsPerPiece,
      extraTraysFromPieces,
      leftoverPieces,
      looseMessage,
